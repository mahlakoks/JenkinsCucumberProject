...
/*
    ...
    @And("User is able to see categories")
    public void userIsAbleToSeeCategories(DataTable dataTable) {
        List<String> categories=dataTable.asList();
        loginPage.validateHeaderCategories(categories);
    }

    @Then("Asser number of items is {int}")
    public void asserNumberOfItemsIs(int itemCount) {
        Assert.assertEquals(itemCount,loginPage.getMainCatCount());
    }
}
